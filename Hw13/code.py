...
# HTTPMethod = adafruit_httpserver.methods.Method
MIMEType = adafruit_httpserver.mime_types.MIMETypes
...
@server.route("/")
def base(request: adafruit_httpserver.server):
    #  serve the HTML f string
    #  with content type text/html
    with adafruit_httpserver.response(request, content_type=adafruit_httpserver.mime_type.TYPE_HTML) as response:
...
def buttonpress(request: adafruit_httpserver.server):
    print("button request")
    #  get query params (doesn't handle all cases, use with caution
    query = {x[0]: x[1] for x in [
        x.split("=") for x in request.body.decode("utf8").split("&")]}
...
@server.route("/temp")
def temp(request: adafruit_httpserver.server):
    print("Temp request")
    # Read temperature
    temp = str(c_to_f(microcontroller.cpu.temperature))
